...
const BannerCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await BannerService.insert(formDataToSend);
      if (response.status) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-100 rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-6 text-center text-blue-500">Thêm Banner</h2>
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            <label htmlFor="name" className="block mb-1 font-medium">Tên banner</label>
            <input
              type="text"
              id="name"
              name="name"
          ...
          <div>
            <label htmlFor="link" className="block mb-1 font-medium">Liên kết</label>
            <input
              type="text"
              id="link"
              name="link"
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor="position" className="block mb-1 font-medium">Vị trí</label>
            <select
              id="position"
              name="position"
              ...
              {banners.map((banner) => (
                <option key={banner.id} value={banner.position}>
                  {banner.position}
                </option>
              ))}
            </select>
          ...
          <div>
            <label htmlFor="image" className="block mb-1 font-medium">Hình</label>
            <input
              type="file"
              id="image"
              name="image"
      ...
      {showConfirmDialog && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-500 bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h2 className="text-lg font-semibold mb-3">Thêm banner thành công</h2>
            <p className="mb-3">Banner đã được thêm thành công. Bạn có muốn tải lại trang?</p>
