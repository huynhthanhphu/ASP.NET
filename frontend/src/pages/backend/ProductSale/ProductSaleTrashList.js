...
const ProductSaleTrashList = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (Array.isArray(result.product_sale)) {
        setProducts(result.product_sale);
      } else {
  ...
  const handleRestore = async (id) => {
    ...
    if (confirmRestore) {
      const response = await ProductSaleService.restore(id);
      if (response.status) {
  ...
  const handleDeleteForever = async (id) => {
    ...
    if (confirmDelete) {
      const response = await ProductSaleService.destroy(id);
      if (response.status) {
        setProducts(products.filter(product => product.id !== id));
      } else {
        alert(response.message);
    ...
    <div className="container mx-auto mt-5 p-4">
      <div className="flex justify-between mb-4">
        ...
        <div className="flex space-x-2">
          <Link to="/admin/product-sale" className="bg-blue-500 text-white px-3 py-1 rounded inline-flex items-center">
  ...
  export default ProductSaleTrashList;