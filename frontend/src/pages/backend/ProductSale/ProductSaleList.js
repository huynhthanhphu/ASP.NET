...
const ProductSaleList = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (Array.isArray(result.product_sale)) {
        setProducts(result.product_sale); // Cập nhật sản phẩm từ phản hồi
      } else {
  ...
  const handleEdit = (id) => {
    navigate(`/admin/product-sale/update/${id}`); // Điều hướng đến trang chỉnh sửa
  };
  const handleShow = (id) => {
    navigate(`/admin/product-sale/show/${id}`); // Điều hướng đến trang chỉnh sửa
  };

  // Hàm xử lý toggle trạng thái sản phẩm
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await ProductSaleService.status(id); // Gọi API cập nhật trạng thái
      if (response.status) {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      const response = await ProductSaleService.delete(id);
      if (response.status) {
  ...
  const handleTrashClick = () => {
    navigate('/admin/product-sale/trash'); // Chuyển hướng đến trang thùng rác
  };

  return (
    <div className="container mx-auto mt-5 p-4">
      <div className="flex justify-between mb-4">
        <h1 className="text-2xl font-bold text-blue-400">Sản phẩm giảm giá</h1>
        <div className="flex space-x-2">
          <Link to="/admin/product-sale/create" className="bg-green-500 text-white px-3 py-1 rounded inline-flex items-center text-sm">
        ...
        <table className="min-w-full shadow-md rounded-lg bg-white border p-6">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="py-2 px-4 border">Giá giảm</th>
            ...
            {products.length > 0 ? (
              products.map((product) => (
                <tr key={product.product_id}>
                  ...
                  <td className="py-2 px-4 border text-center">{product.price_sale}</td>
                  <td className="py-2 px-4 border text-center">{product.date_begin}</td>
                  <td className="py-2 px-4 border text-center">{product.date_end}</td>
