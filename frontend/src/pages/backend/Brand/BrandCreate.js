...
const BrandCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await BrandService.insert(formDataToSend);
      if (response.status) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-100 rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-6 text-center text-blue-500">Thêm Thương Hiệu</h2>
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            <label htmlFor="name" className="block mb-1 font-medium">Tên thương hiệu</label>
            <input
              type="text"
              id="name"
              name="name"
      ...
      {showConfirmDialog && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-500 bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h2 className="text-lg font-semibold mb-3">Thêm thương hiệu thành công</h2>
            <p className="mb-3">Thương hiệu đã được thêm thành công. Bạn có muốn tải lại trang?</p>
