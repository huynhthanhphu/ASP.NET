...
const BrandUpdate = () => {
    ...
    useEffect(() => {
        const fetchBrandData = async () => {
            try {
                ...
                if (response.status) {
                    const brandData = response.brand;
                    setFormData({
                        name: brandData.name,
                        description: brandData.description,
                        sort_order: brandData.sort_order,
                        status: brandData.status.toString(),
                    });
                    setCurrentImage(brandData.image);
                } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await BrandService.update(id, formDataToSend);
            if (response.status) {
            ...
            toast.error(`Có lỗi xảy ra khi cập nhật brand: ${error.message || error}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-4xl mx-auto p-4">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold">Cập nhật Thương hiệu</h2>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-6">
                <div className="space-y-4">
                    <div>
                        <label htmlFor="name" className="block mb-1">Tên thương hiệu</label>
                        <input
                            type="text"
                            id="name"
                            name="name"
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        {currentImage && (
                            <img src={currentImage} alt="Current brand" className="w-full h-32 object-cover mb-2" />
                        )}
                        <input
                            type="file"
                            id="image"
            ...
            {showConfirmDialog && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        ...
                        <p className="mb-3">Thương hiệu đã được cập nhật thành công. Bạn có muốn tải lại trang?</p>
